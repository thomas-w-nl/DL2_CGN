...
class U2NET(nn.Module):
    ...
    def initialize(cls, weight_path=None):
        '''init and optionally load weights'''
        model = U2NET(3, 1)
    ...
    def forward(self, x):
        ...
        return torch.sigmoid(d0)  # sigmoid limits the range to [0-1], but images have been normalized
        # return d0


##### U^2-Net ####
class RefineNetShallow(nn.Module):
    """
    ...
    def forward(self, x):
        ...
        residuals = self.relu(self.conv2(residuals))
        residuals = self.conv3(residuals)
